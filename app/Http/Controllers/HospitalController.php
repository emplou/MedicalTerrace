...
class HospitalController extends Controller
{
    ...
    public function save_copy_add_hospital(Request $request){
        ...
        $hospital->fax              = $details['fax'].'-'.$details['fax_one'].'-'.$details['fax_two'];
        $hospital->email            = $details['email'];
        $hospital->image            = '/medicalterrace/public/clinic_image/'.$filename; 
        $hospital->image_caption    = $details['img_caption'];
        $hospital->image_alt        = $details['img_alt'];
        $hospital->hosp_subheading  = $details['hosp_subheading']; 
        $hospital->hosp_text_subheading  = $details['textheading_lead']; 
        $hospital->division         = $details['division']; 
        $hospital->medsublist       = $jsonsubj_list; 

        $hospital->url_hosp         = $details['url_hosp'];
        $hospital->in_hospital_pres = $details['pres'];
        $hospital->free_med_exp     = $med_json; 
        $hospital->hospitalization  = $hosp_json; 
        $hospital->no_of_beds       = $bed_json; 
        $hospital->possible_date_of_visit = $visit_json; 
        $hospital->in_hospital_services   = $details['hosp_service']; 
        $hospital->support_lang     = $details['support_lang'];
        $hospital->shop_dining_room = $details['shop_dining_room']; 
        $hospital->credit_card_payment    = $bed_json; 
        $hospital->tracking_status    = '1'; 
        $hospital->author           = $author; 
        $hospital->save();
